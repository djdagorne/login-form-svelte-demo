...
<style>
	...
	.label {
		z-index: -1;
		position: absolute;
		transform: translateY(-2rem); /* moving the label to hover over the input */
		transform-origin: 0%;
		transition:  400ms;
	}

	.field:focus-within .label,
	.input:not(:placeholder-shown) + .label{ /* keeps the transform of the label when input is not = placeholder (which should be " ", not "") */
